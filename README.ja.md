...
- Strength : Weak / Medium / Strong / Custom からフィルタの強さを選択する。`Window > Polyorder` が必須条件のため、これを満たさないとエラーになる。
- Window : 計算に使用するデータ点数を指定する。`3, 5, 7, 9, 11`の中から指定可能。大きすぎる場合、数値の細かな特徴が失われる。小さすぎる場合、ノイズが十分に除去されない。
- Polyorder : 近似に用いる多項式の次数。`2, 3, 4`の中から指定可能。高すぎる場合、過剰なフィッティングに伴い、新たなノイズが生じる可能性がある。低すぎる場合、平滑化されすぎるため、大きな動きに追従できない可能性がある。
